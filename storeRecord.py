...
class StoreRecord:
    ...
    def __init__(self, recordID, recordData, lastSavedByInstance, lastSavedByCounter, lastSavedByHistory,
                 partitionFacility, partitionUser):
        """
        Constructor
        """
        if len(recordID) == 0:
    ...
    def printStoreRecord(self):
        ...
        """
        print
        "Store Record ID :" + str(self.recordID)
        print
        "Record Data :" + str(self.recordData)
        print
        "Last Saved by Instance & respective Counter position :" + str(self.lastSavedByInstance) + " " + str(
            self.lastSavedByCounter)
        print
        "Last Saved by History : " + str(self.lastSavedByHistory)
        print
        "Partitions Facility:" + str(self.partitionFacility) + " User:" + str(self.partitionUser)
